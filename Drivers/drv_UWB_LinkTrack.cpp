			...
			{	//校验
				if( sum == rdata )
				{
					...
					{					
						...
						if( Uwb.eop[2] > 200 )
							PositionSensorChangeDataType( default_uwb_sensor_index,driver_info.sensor_key, Position_Sensor_DataType_s_xy );
						else
							PositionSensorChangeDataType( default_uwb_sensor_index,driver_info.sensor_key, Position_Sensor_DataType_s_xyz );
						double eop_xy = sqrtf( Uwb.eop[0]*Uwb.eop[0] + Uwb.eop[1]*Uwb.eop[1] );
						PositionSensorUpdatePosition( default_uwb_sensor_index,driver_info.sensor_key, pos, true, -1, eop_xy, Uwb.eop[2] );
					}
				}
				rc_counter = 0;
				sum = 0;
...
static bool UWB_LinkTrack_DriverInit( Port port, uint32_t param )
{
	...
	//注册传感器
	uint32_t sensor_key = PositionSensorRegister( default_uwb_sensor_index , \
																								"UWB_LinkTrack" ,\
