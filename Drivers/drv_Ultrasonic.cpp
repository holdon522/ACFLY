...
static void UltrasonicTCB( void *pvParameter1, uint32_t ulParameter2 )
{
	...
	if( position.z>1 && position.z<1000 )
	{
		...
		PositionSensorUpdatePosition( default_ultrasonic_sensor_index,sensor_key, position, true );
	}
	else
		PositionSensorSetInavailable( default_ultrasonic_sensor_index,sensor_key );
}
...
void init_drv_ultrasonic()
{
	...
	//注册传感器
	sensor_key = PositionSensorRegister( default_ultrasonic_sensor_index , \
																				"Ultrasonic" ,\
