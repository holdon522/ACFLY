...
reTry:
	...
	{		
		...
		res = ExtIIC_SendAddr7( ADDR, tx_buf, 2, IICSpeed_Standard100k );
		if(!res)
			continue;
		os_delay(2);
		...
		res = ExtIIC_SendAddr7( ADDR, tx_buf, 6, IICSpeed_Standard100k );
		if(!res)
			continue;
		...
		res = ExtIIC_SendAddr7( ADDR, tx_buf, 2, IICSpeed_Standard100k );	os_delay(1);
		if(!res)
			continue;
