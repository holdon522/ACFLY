...
/*7位地址发送并接收数据
	...
	Sync_waitTime：超时时间
*/
bool ExtIIC_SendReceiveAddr7( uint8_t addr, const uint8_t* tx_datas, uint16_t tx_length, const uint8_t* rx_datas, uint16_t rx_length, IICSpeed speed, double Sync_waitTime )
{
...
void init_drv_ExtIIC()
{
	...
	/*配置GPIO PB8(IIC1 SCL) PB9(IIC1 SDA)*/
		...
		//引脚开漏上拉
		set_register( GPIOB->OTYPER , 1 , 8*1 , 1 );	//PB8
