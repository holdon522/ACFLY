...
{	
	...
	switch( inf->counter1 )
	{
		...
		{	//判断执行旋转偏航
			...
			{	//指定偏航朝向
				Attitude_Control_set_Target_YawRelative( degree2rad(params[3]) );
				inf->counter1 = 1;
				inf->counter2 = 0;
			}
			else
		...
		{	//等待偏航旋转开始航点飞行
			...
			if( wp_range > 10.0f )
				yaw_track_err_scale = wp_range * 0.5f;
			...
			{		
				...
				switch(frame)
				{
					...
					case MAV_FRAME_GLOBAL_TERRAIN_ALT_INT:
					{
						...
						{
							vector3<double> target_pos;
							get_TargetPosInf( 0, 0, &target_pos, 0 );
		...
		{	//等待航点飞行完成
			...
			if( frame==MAV_FRAME_GLOBAL_TERRAIN_ALT || frame==MAV_FRAME_GLOBAL_TERRAIN_ALT_INT ) {
				...
				{	//根据测高传感器调整对地高度
					vector3<double> pos;
					get_Position_Ctrl(&pos);
					double err = params[6]*100 - sensor.position.z;
					...
					Position_Control_set_RouteLineAvoidanceRelative( maxFrontPointL );
				}
				else
				{	//无高度传感器锁定高度
					Position_Control_set_ZLock();
...
*/
static int16_t NavCmd192_DO_REPOSITION( double freq, uint8_t frame, double params[], NavCmdInf* inf )
{
	if( get_Position_MSStatus() != MS_Ready )
	{	//无定位出错
		inf->counter1 = inf->counter2 = 0;
			...
			{		
				...
				switch(frame)
				{
					...
					case MAV_FRAME_GLOBAL:
					{
						...
						{	//经纬度不正确只进行高度调整
							Position_Control_set_XYLock();
							res = Position_Control_set_TargetPositionZGlobal( params[6]*100, vel );
						}
						else	//经纬度正确进行三维飞行
							res = Position_Control_set_TargetPositionXYZ_LatLon( params[4], params[5], params[6]*100, vel );
						break;
					}
					case MAV_FRAME_GLOBAL_RELATIVE_ALT_INT:
					case MAV_FRAME_GLOBAL_RELATIVE_ALT:
					{
						...
						{	//经纬度不正确只进行高度调整
							Position_Control_set_XYLock();
							res = Position_Control_set_TargetPositionZRA( params[6]*100, vel );
						}
						else	//经纬度正确进行三维飞行
							res = Position_Control_set_TargetPositionXYZRA_LatLon( params[4], params[5], params[6]*100, vel );
						break;
					}
					case MAV_FRAME_GLOBAL_TERRAIN_ALT:
					case MAV_FRAME_GLOBAL_TERRAIN_ALT_INT:
					...
					case MAV_FRAME_LOCAL_NED:
						res = Position_Control_set_TargetPositionXYZ( params[5]*100, params[4]*100, -params[6]*100, vel );
						break;
					
					case MAV_FRAME_LOCAL_ENU:
						res = Position_Control_set_TargetPositionXYZ( params[4]*100, params[5]*100, params[6]*100, vel );
						break;
					
					case MAV_FRAME_LOCAL_OFFSET_NED:
						res = Position_Control_set_TargetPositionXYZRelative( params[5]*100, params[4]*100, -params[6]*100, vel );
						break;
					...
					case MAV_FRAME_BODY_OFFSET_NED:
					{
						//body系修正航向后只向前走
						if( isvalid(params[3]) == false )
							res = Position_Control_set_TargetPositionXYZRelativeBodyheading( safe_sqrt(params[5]*params[5] + params[4]*params[4])*100, 0, -params[6]*100, vel );
						else
							res = Position_Control_set_TargetPositionXYZRelativeBodyheading( params[5]*100, params[4]*100, -params[6]*100, vel );
						break;
					}
					...
					case MAV_FRAME_BODY_FLU:
					{
						//body系修正航向后只向前走
						if( isvalid(params[3]) == false )
							res = Position_Control_set_TargetPositionXYZRelativeBodyheading( safe_sqrt(params[4]*params[4] + params[5]*params[5])*100, 0, params[6]*100, vel );
						else
							res = Position_Control_set_TargetPositionXYZRelativeBodyheading( params[4]*100, params[5]*100, params[6]*100, vel );
						break;
					}
...
TurnYawLocalP:
			...
			double RtRotateRange = 1000;
			if( sq(LA) + sq(LB) > sq(RtRotateRange) )
				Attitude_Control_set_Target_Yaw( atan2(LA,LB) );
			inf->counter1 = 1;
			inf->counter2 = 0;
...
/*NavCmd115_CONDITION_YAW
	...
	参数:
		...
		<param index="7">Empty</param>
*/
static int16_t NavCmd115_CONDITION_YAW( double freq, uint8_t frame, double params[], NavCmdInf* inf )
{
	if( get_Position_MSStatus() != MS_Ready )
	{	//无定位出错
		inf->counter1 = inf->counter2 = 0;
