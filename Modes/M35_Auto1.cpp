		...
		{	//任务模式
			...
			{	//任务飞行				
				...
				switch( mission_ind )
				{
					...
					case 1:
					{
						//Position_Control_set_TargetVelocityXY_AngleLimit(15,0,0,5);
						Position_Control_set_XYZAutoSpeed(20);
//						double params[7];
//						params[0] = 1;
//						params[1] = 0;
//						params[2] = 0;
//						params[3] = nan("");
//						params[4] = 2;	params[5] =0 ;
//						params[6] = 0;
//						int16_t res = Process_NavCmd( MAV_CMD_NAV_WAYPOINT, freq, MAV_FRAME_BODY_FLU, params, &navInf );
						Position_Control_set_TargetVelocityBodyHeadingXY_AngleLimit( 15,0 , 0, 3);
						
						if(msg_available)
						{	
							
							if( msg.cmd == 2 )		//识别到元素刹车
							{
							Position_Control_set_XYLock();
							Position_Control_set_ZLock();
							++mission_ind;
					...
					{	//飞直线
						...
						{		
							if( msg.cmd == 2 )
							{
									...
									double cx = msg.params[0];
									double cy = msg.params[1];
										//		Position_Control_move_TargetPositionXYRelativeBodyheading(cx,cy);
									if(fabs(cx)>1&&fabs(cy)>1)
									Position_Control_set_TargetVelocityBodyHeadingXY_AngleLimit( cx,cy , 0.05, 0.05);
									else ++mission_ind;
							}
...
//					{	
//						Position_Control_set_XYLock();
//						Position_Control_Takeoff_HeightRelative(-800);
//						Position_ControlMode pos_mode;
//						get_Altitude_ControlMode(&pos_mode);
...
//					}
					case 6:						//起飞
					{
						...
						{	//起飞完成
							init_NavCmdInf(&navInf);
							Attitude_Control_set_Target_YawRelative(degree2rad(-180.0));				//先右转180防止障碍干扰
							Position_Control_set_ZLock();
							Position_Control_set_XYLock();
					...
					Attitude_Control_set_Target_YawRelative_Offboard(degree2rad(180.0),0.2);
					}
					else{

					Attitude_Control_set_Target_YawRelative(degree2rad(0.0));
					Position_Control_set_ZLock();
					Position_Control_set_XYLock();
					dis =distance;
					Attitude_Control_set_YawLock();
					...
					if(distance>300)
					{
							Position_Control_set_ZLock();
							Position_Control_set_XYLock();
							Attitude_Control_set_Target_YawRate(0.2);
					...
					else 
					{
						...
						dis =distance;
						++mission_ind;
				//		Position_Control_set_TargetVelocityBodyHeadingXY_AngleLimit(0,-5,1,1);
					...
					case 8:						//接近立柱
					{
						Position_Control_set_XYZAutoSpeed(15);
							dis1=(dis-50)/100;
							double params[7];
							params[0] = 1;
							params[1] = 0;
