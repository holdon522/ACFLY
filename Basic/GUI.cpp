	...
	while(1)
	{
		...
		/*XY轴*/
			...
			if( get_Health_XY(&posinfxy) )
			{
				...
				health = 0;
			//printf( str, "%3.0f%%", health );
				sprintf( str, "%3.1f", dis1 );
		...
		/*位置传感器状态*/
			...
			{
				Position_Sensor_Data sensor;
				if( GetPositionSensorData( i+8, &sensor ) )
					if( sensor.available )
