...
static void Msg69_MANUAL_CONTROL( uint8_t Port_index , const mavlink_message_t* msg )
{
	...
	for( uint8_t i = 4; i < 16; ++i )
		raw_data[i] = ( msg_rd->buttons&(1<<(i-4)) ) ? 100 : 0;
...
static void Msg102_VISION_POSITION_ESTIMATE( uint8_t Port_index , const mavlink_message_t* msg )
{
	...
	static double last_vel_y =0;
	
	// Çóµ¼Çó³öËÙ¶È
	double h = updateTime.get_pass_time_st();
	double vel_x = (msg_rd->x * 100 - last_vel_x)/h; 
	double vel_y = (-msg_rd->y * 100 - last_vel_y)/h; 
	last_vel_x = msg_rd->x * 100;
	last_vel_y = -msg_rd->y * 100;
	
	vector3<double> VelSlam;
	fast_sin_cos(-msg_rd->yaw, &sinYaw, &cosYaw);
	VelSlam.x = ENU2BodyHeading_x( vel_x , vel_y , sinYaw , cosYaw );
	VelSlam.y = ENU2BodyHeading_y( vel_x , vel_y , sinYaw , cosYaw );
  PositionSensorUpdateVel( default_vslam_pos_index,vslam_pos_register_key, VelSlam, true );

//	Quaternion quat;
//	get_Attitude_quat(&quat);	
//	extern float debug_test[30];
//	debug_test[11]= VelSlam.x;
//	debug_test[12]= VelSlam.y;	
//	debug_test[13]= msg_rd->x;
//	debug_test[14]= msg_rd->y;
//	debug_test[15]= -rad2degree(msg_rd->yaw);
//	debug_test[16]= rad2degree(quat.getYaw());
